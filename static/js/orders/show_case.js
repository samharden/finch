...
$("#comment_form").submit(function(e){
    ...
    $.ajax({
                url : "/questions/create_comment/", // the endpoint,commonly same url
            ...
            for (var key in data.file){
                    ...
                    "<span class='preview'><a href='/questions/"+key+"download'>"+data.file[key]+"</a></span>"+
...
$("#comment_edit").click(function(e){
    ...
    $.ajax({
                url : "/questions/edit_comment/", // the endpoint,commonly same url
                type : "POST", // http method
                data : formData,
                cache: false,
                contentType: false,
...
function remove_file(x){
    ...
    if (con == true){
    $.post('/questions/remove_comment_file/', {"file_id":x, "csrfmiddlewaretoken": csrftoken,}, function(data){
        if(data.error){
            alert(data.error)
        }
        else{
...
function remove_comment(x){
    ...
    var con = confirm("Sure, You want to delete")
    if (con == true){
    $.post('/questions/remove_comment/', {"comment_id":x, "csrfmiddlewaretoken": csrftoken,}, function(data){
        if(data.error){
         alert(data.error)
        }
        else{
...
function editFun(x) {
    ...
    $.ajax({
        type: "POST",
        url: "/questions/editdetails/", // the endpoint,commonly same url
        data: {
